...
- Desde `src/cli/main.py setup-data`, puedes decidir si ese error corta el flujo con `--strict-download`.
...
```bash
uv run python src/scripts/setup/bootstrap_local_data.py
```

Solo verificacion:

```bash
uv run python src/scripts/setup/bootstrap_local_data.py --verify-only
```

Descarga asistida de HaGRID (simulación):

```bash
uv run python src/scripts/setup/download_hagrid_kaggle.py
```

Descarga real y preparación de anotaciones:

```bash
uv run python src/scripts/setup/download_hagrid_kaggle.py --execute --prepare-ann-subsample
```
...
```bash
uv run python src/scripts/setup/download_hagrid_kaggle.py --ask-credentials --execute
```
