...
OUT_DIR = BASE / "output" / "graficas_avanzadas"
OUT_DIR.mkdir(parents=True, exist_ok=True)

import sys
sys.path.insert(0, str(BASE / "src" / "stgcn"))
from hand_graph import build_adjacency_matrix
from stgcn_model import STGCN

MANIFEST_PATH = BASE / "output" / "manifest_unificado_final.csv"
HISTORY_PATH = BASE / "output" / "training_history_canonico.json"
CHECKPOINT_PATH = BASE / "output" / "best_stgcn_canonico.pth"
...
def graph_07_kine_kinemorfo(df_test: pd.DataFrame):
    first_path = BASE / df_test.iloc[0]["path_secuencia"].replace("\\", "/")
...
def graph_08_heatmap_importance(df_test: pd.DataFrame):
    ...
    for p in sample_paths[:1200]:
        seq = np.load(str(BASE / p.replace("\\", "/")))  # (T,21,3)
        stack.append(seq)
...
def main():
    ...
    dataset = GestureDataset(df_test, class_to_idx, BASE)
