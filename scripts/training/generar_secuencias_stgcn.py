...
def generar_secuencia_sintetica(
    ...
    """
    # Frame base replicado T veces
    secuencia = np.tile(landmarks_estaticos, (T, 1, 1))  # (T, 21, 3)

    # Ruido motor: cada articulación tiene trayectoria levemente diferente
    ruido = np.random.normal(0, sigma_ruido, size=(T, 21, 3))

    # Suavizar el ruido en el tiempo para que sea físicamente plausible
    for nodo in range(21):
        for coord in range(3):
            ruido[:, nodo, coord] = np.convolve(
                ruido[:, nodo, coord],
                np.ones(3) / 3,  # media móvil de ventana 3
                mode='same'
            )
...
def procesar_manifest(csv_path: str, output_dir: str, T: int = 16, seed: int = 42):
    ...
    for idx, (_, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="Generando secuencias")):
        try:
            ...
            secuencia = generar_secuencia_sintetica(landmarks, T=T)  # (T, 21, 3)
